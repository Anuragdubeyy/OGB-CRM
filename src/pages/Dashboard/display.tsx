...
import { displayColumn, overviewColumn } from "./column";
...
import { getDashboardDisplayListAsync, selectAllDashboardDisplayList } from "../../store/slices/dashboardDisplay";
interface ChartData {
  labels: string[];
  datasets: {
    label: string;
...
export default function DisplayDashboard() {
  ...
  const displayList = useAppSelector(selectAllDashboardDisplayList);

  useEffect(() => {
    dispatch(getDashboardOverviewListAsync());
    dispatch(getDashboardDisplayListAsync());
  }, [dispatch]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const url = buildUrlWithParams(API_URL.GET_GOLD_SELL_CHART, {
  ...
  return (
    <div>
      ...
      <div className="flex gap-6  mt-8">
        ...
        <DataTable tableRef={[]} columns={displayColumn} data={displayList} />
      </div>
