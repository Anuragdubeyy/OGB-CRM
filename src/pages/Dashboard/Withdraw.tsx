...
import { getDashboardWithdrawListAsync, selectAllDashboardWithdrawList } from "../../store/slices/dashboardWithdraw";
import { overviewColumn, withdrawColumn } from "./column";
interface ChartData {
  labels: string[];
  datasets: {
    label: string;
...
export default function WithdrawDashboard() {
  ...
  const  withdrawList = useAppSelector(selectAllDashboardWithdrawList);

  useEffect(() => {
    dispatch(getDashboardOverviewListAsync());
    dispatch(getDashboardWithdrawListAsync());
  }, [dispatch]);
  ...
  return (
    <div>
      ...
      <div className="flex gap-6  mt-8">
        ...
        <DataTable tableRef={[]} columns={withdrawColumn} data={withdrawList} />
      </div>
