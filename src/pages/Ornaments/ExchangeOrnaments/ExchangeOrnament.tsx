...
import { exchangeOrnamentColumn } from "./column";
import {useDownloadExcel} from 'react-export-table-to-excel'

export default function ExchangeOrnaments() {

  const tableRef=useRef<null>(null)

  const {onDownload}=useDownloadExcel({
    currentTableRef:tableRef.current,
    filename:'exchange_ornaments',
    sheet:"data" 
  })
  ...
  return (
    <section className="border-2 rounded-3xl p-2" style={{borderColor: "rgba(195, 166, 109, 1)"}}>
      <div className="flex justify-between p-2">
        <div>
          <h2 className="font-bold text-black ">Exchange Ornaments</h2>
          <p className=" text-black  text-opacity-70">Total Exchange Ornaments : 9</p>
        </div>
  ...
  <DataTable tableRef={tableRef} columns={exchangeOrnamentColumn} data={[]}/>
</div>
