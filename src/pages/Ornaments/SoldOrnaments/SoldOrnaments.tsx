...
import { soldOrnamentColumn } from "./column";
import { useAppDispatch, useAppSelector } from "../../../store/Hooks";
import { getSoldOrnamentsListAsync, selectAllSoldOrnamentsList } from "../../../store/slices/soldOrnaments";
...
export default function SoldOrnaments() {
  ...
  const {onDownload}=useDownloadExcel({
    ...
    filename:'sold-ornaments',
    sheet:"data" 
  })
  
  const dispatch = useAppDispatch();

  const soldOrnamentsList = useAppSelector(selectAllSoldOrnamentsList);


  useEffect(() => {
    dispatch(getSoldOrnamentsListAsync());
  }, [dispatch]);
  return (
    <section className="border-2 rounded-3xl p-2" style={{borderColor: "rgba(195, 166, 109, 1)"}}>
      <div className="flex justify-between p-2">
        <div>
          ...
          <p className=" text-black  text-opacity-70">Total Sold Ornaments :{soldOrnamentsList.length} </p>
        </div>
  ...
  <DataTable tableRef={tableRef} columns={soldOrnamentColumn} data={soldOrnamentsList}/>
</div>
