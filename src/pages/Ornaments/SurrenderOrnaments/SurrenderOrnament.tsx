...
import { surrenderOrnamentColumn } from "./column";
import {useRef} from 'react'
import {useDownloadExcel} from 'react-export-table-to-excel'

export default function SurrenderOrnaments() {
  const tableRef=useRef<null>(null)

  const {onDownload}=useDownloadExcel({
    currentTableRef:tableRef.current,
    filename:'surrender-ornaments',
    sheet:"data" 
  })
  ...
  return (
    <section className="border-2 rounded-3xl p-2" style={{borderColor: "rgba(195, 166, 109, 1)"}}>
      <div className="flex justify-between p-2">
        <div>
          <h2 className="font-bold text-black ">Surrender Ornaments</h2>
          <p className=" text-black  text-opacity-70">Total Surrender Ornaments : 9</p>
        </div>
  ...
  <DataTable tableRef={tableRef} columns={surrenderOrnamentColumn} data={[]}/>
</div>
