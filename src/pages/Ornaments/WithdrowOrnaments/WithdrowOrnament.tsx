...
import { withdrowOrnamentColumn } from "./column";
import { getWithdrawnOrnamentsListAsync, selectAllWithdrawnOrnamentsList } from "../../../store/slices/withdrawnOrnaments";
import { useEffect ,useRef} from "react";
import { useAppDispatch, useAppSelector } from "../../../store/Hooks";
import {useDownloadExcel } from "react-export-table-to-excel"

export default function WithdrowOrnaments() {
  const tableRef=useRef<null>(null)

  const {onDownload}=useDownloadExcel({
    currentTableRef:tableRef.current,
    filename:'withdraw-ornaments',
    sheet:"data" 
  })
  
  const dispatch = useAppDispatch();

  const withdrawnOrnamentsList = useAppSelector(selectAllWithdrawnOrnamentsList);


  useEffect(() => {
    dispatch(getWithdrawnOrnamentsListAsync());
  }, [dispatch]);
  return (
    <section className="border-2 rounded-3xl p-2" style={{borderColor: "rgba(195, 166, 109, 1)"}}>
      <div className="flex justify-between p-2">
        <div>
          <h2 className="font-bold text-black ">Withdrown Ornaments</h2>
          <p className=" text-black  text-opacity-70">Total Withdrown Ornaments : {withdrawnOrnamentsList ? withdrawnOrnamentsList.length : 0} </p>
        </div>
  ...
  <DataTable tableRef={tableRef} columns={withdrowOrnamentColumn} data={withdrawnOrnamentsList}/>
</div>
